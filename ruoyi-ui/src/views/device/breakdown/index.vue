...
<script>
import {ListBreakdown, ListCompanyName} from "@/api/device/breakdown"
...
export default {
  ...
  data: function () {
    return {
      ...
      uploadHeaders: { Authorization: 'Bearer ' + sessionStorage.getItem('token') },//设置上传文件的请求头
      loading: true,
  ...
  methods: {
    ...
    listBreakdown(){
      ...
      //   let res=response.result.dataList
      //   console.log(res)
      // }).catch(error => {
      //   console.log(error)
      // })
      ...
      axios({
        url: 'http://139.9.229.132:8282/device/report/host/getHostList',
        method: 'post',
        params: {
          page: this.current,
          pageSize:this.size,
        ...
        .then(function (response) {
          this.data().breakdowns=response.data.result.dataList
          console.log(this.data().breakdowns)
        }).catch(function (err) {
        console.log(err)
      });
    },
